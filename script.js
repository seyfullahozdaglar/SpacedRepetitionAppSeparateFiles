...
function applyNightMode(enable) {
    ...
    // Update gender background when toggling night mode
...
function updateGenderBackground() {
    // Remove all gender classes first
    document.body.classList.remove('gender-masculine', 'gender-feminine', 'gender-neutral', 'gender-default');
    
    // Only apply gender background in practice view
    if (views.practice.classList.contains('active')) {
        const card = currentSession.cards[currentSession.currentIndex];
        if (card && card.gender) {
            document.body.classList.add(`gender-${card.gender}`);
        } else {
            document.body.classList.add('gender-default');
        }
    } else {
        document.body.classList.add('gender-default');
    }
...
function showView(viewName) {
    ...
    // Reset gender background when leaving practice view
    if (viewName !== 'practice') {
        document.body.classList.remove('gender-masculine', 'gender-feminine', 'gender-neutral');
        document.body.classList.add('gender-default');
    } else {
        // Update gender background when entering practice view
        updateGenderBackground();
    }
}
...
function showPracticeCard() {
    ...
    // Update gender background
    updateGenderBackground();

    // Display gender if available
    if (card.gender && card.gender.trim() !== '') {
        genderDisplay.textContent = `Gender: ${card.gender}`;
...
function parseAndImport(content) {
    ...
    lines.forEach(line => {
        ...
        if (parts.length >= 3) {
            // Check if the third field could be a gender
            const possibleGender = parts[2].toLowerCase();
            if (['masculine', 'feminine', 'neutral', 'm', 'f', 'n'].includes(possibleGender)) {
                ...
                // Remaining fields
                if (parts.length === 4) note = parts[3];
                else if (parts.length >= 5) { image = parts[parts.length-1]; note = parts.slice(3,parts.length-1).join(' // '); }
            } else {
                // Third field is not a gender, treat it as note
                note = parts[2];
                if (parts.length === 4) image = parts[3];
                else if (parts.length >= 5) { image = parts[parts.length-1]; note = parts.slice(2,parts.length-1).join(' // '); }
            }
        }
...
// Export metadata (CSV) — includes gender and audio columns
...
// Import metadata CSV — maps gender and audio fields
