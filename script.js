...
function applyGenderTheme(gender) {
    // Remove all gender theme classes
    document.body.classList.remove('theme-feminine', 'theme-neutral');
    
    // Apply appropriate theme based on gender
    if (gender === 'feminine') {
        document.body.classList.add('theme-feminine');
    } else if (gender === 'neutral') {
        document.body.classList.add('theme-neutral');
    }
    // No theme class for masculine or no gender (uses default)
}

// Remove gender theme when leaving practice view
function removeGenderTheme() {
    document.body.classList.remove('theme-feminine', 'theme-neutral');
}
...
function showStatistics(filter = 'availableNow') {
    ...
    filteredCards.forEach(card => {
        ...
        const genderDisplay = card.gender || '';
        row.innerHTML = `
            <td>${card.word}</td>
            <td>${card.meaning}</td>
            <td>${genderDisplay}</td>
...
function parseAndImport(content) {
    ...
    lines.forEach(line => {
        ...
        if (parts.length === 3) {
            // Could be gender or note - check if it's a valid gender
            const thirdPart = parts[2].toLowerCase();
            if (thirdPart === 'masculine' || thirdPart === 'feminine' || thirdPart === 'neutral') {
                gender = thirdPart;
            } else {
                note = parts[2];
            }
            ...
            if (gender !== 'masculine' && gender !== 'feminine' && gender !== 'neutral') {
                gender = '';
                note = parts.slice(2, parts.length-1).join(' // ');
            } else {
                note = parts.slice(3, parts.length-1).join(' // ');
            }
...
function setupEventListeners() {
    ...
    document.getElementById('backToDashboardBtn').addEventListener('click', ()=> { removeGenderTheme(); showView('dashboard'); });
    document.getElementById('summaryBtn').addEventListener('click', ()=> { removeGenderTheme(); showView('dashboard'); });
    document.getElementById('backFromStatsBtn').addEventListener('click', ()=> { removeGenderTheme(); showView('dashboard'); });
    document.getElementById('backFromImportBtn').addEventListener('click', ()=> { removeGenderTheme(); showView('dashboard'); });
